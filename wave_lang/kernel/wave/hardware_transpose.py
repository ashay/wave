...
def modify_index_for_full_coverage(original_index: dict) -> dict:
    ...
    for dim, index_seq in original_index.items():
        if dim.name == 'N':
            # modified_index[dim] = IndexSequence(sympy.floor(THREAD_0 / 4), 8, 1)
            modified_index[dim] = IndexSequence(sympy.floor(THREAD_0 / 2), 8, 1)
            """
            ...
            # modified_index[dim] = IndexSequence(sympy.Mod(THREAD_0, 4) * 8, 1, 1)
            modified_index[dim] = IndexSequence(sympy.Mod(THREAD_0, 2) * 8, 1, 1)
            """
            start_expr = index_seq.start
...
def mark_hw_transpose(write: Write, new_writes: dict, read: Read, new_reads, constraints):
    with write.graph.inserting_before(write.fx_node):
        ...
        dest.update_arg("hardware_transpose", True)

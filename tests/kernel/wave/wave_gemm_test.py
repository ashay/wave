...
def testi8NonTransposeGemm(
    shape: tuple[int], mfma_variant: MMAType, request
):
...
def testi8TransposeGemm(
    ...
    randint_hi = 4
    a = device_randint(randint_hi, (shape[0], shape[2]), device='cuda', dtype=torch.int8)
    b = device_randint(randint_hi, (shape[1], shape[2]), device='cuda', dtype=torch.int8)
    c = device_zeros(shape[0], shape[1], dtype=torch.int32)
    asm = gemm(a, b, c)
...
def testF16Transpose(shape: tuple[int], mfma_variant: MMAType):
    # Input sizes
    M = tkl.sym.M
    N = tkl.sym.N
    K = tkl.sym.K
    ...
    a = device_randn(shape[0], shape[2], dtype=torch.float16)
    b = device_randn(shape[2], shape[1], dtype=torch.float16)
    c = device_randn(shape[0], shape[1], dtype=torch.float32)
    gemm(a, b, c)

    torch_ref = torch.matmul(a.cpu().to(torch.float16), b.cpu().to(torch.float16))
